...
class EcosystemConfig(Config):
    ...
    Prey_Reproduction_Rate = 0.0030
    prey_energy_level = 5 * 60
    
    # Fox Config
    predator_movement_speed = 25
    stochastic_death_Rate = 0.0001
    default_energy_level = 4.5 * 60
    hunger_threshold = 0.6 * default_energy_level   # Threshold for when the predator starts to chase (hungry)
    Predator_Reproduction_Rate = 0.035
    eating_distance = 16
...
class Prey(Agent):
    ...
    def find_grass(self):
        if self.config.prey_energy_level * 0.45 < self.energy:
            return False
...
for i in range(3):
    ...
    df_raw.write_csv(f"Assignment 2/Results/PreyPredator_Grass{i}.csv")
    plot = sns.relplot(x = df_raw["frame"], y = df_raw["AgentCount"], hue = df_raw["AID"], kind = "line")
    plot.savefig(f"Assignment 2/Results/PreyPredator_Grass{i}.png", dpi = 300)
